...
enum class SpecialKey {
    SHIFT, BACKSPACE, SYMBOL, GLOBE, SPACE, ENTER
}
...
internal fun buildMap(src: String, dst: String): Map<Char, String> {
    ...
    val dstList: List<String> = mutableListOf<String>().apply {
        var i = 0
        while (i < dst.length) {
...
private val DEVA_CONSONANTS = setOf("क", "ख", "ग", "घ", "ङ", "च", "छ", "ज", "झ", "ञ", "ट", "ठ", "ड", "ढ", "ण", "त", "थ", "द", "ध", "न", "प", "फ", "ब", "भ", "म", "य", "र", "ल", "व", "श", "ष", "स", "ह", "ळ", "ऱ", "ऴ", "ऩ", "क़", "ख़", "ग़", "ज़", "ड़", "ढ", "फ़", "य़")
...
object ScriptManager {
    ...
    private fun buildScriptData(script: BrahmiScript): ScriptData {
        val localVowelToMatra = DEVA_VOWELS.zip(DEVA_MATRAS).associate {
        ...
        val filteredLayers = layers.mapValues {
            it.value.map { row -> row.filter { item -> if (item is Pair<*, *>) (item.first as String).isNotEmpty() || (item.second as String).isNotEmpty() else true } }
        }
        return ScriptData(script, localConsonants, localModifiers, localVowelToMatra, filteredLayers)
    }

    private fun transliterateGrid(grid: List<List<Any>>, script: BrahmiScript): List<List<Any>> {
        return grid.map { row -> row.map { item -> if (item is Pair<*, *>) (item.first as String).toBrahmiScript(script) to (item.second as String).toBrahmiScript(script) else item } }
    }
}

fun formatKeyText(raw: String, currentBaseChar: String, scriptData: ScriptData): String {
