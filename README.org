...
- *Landscape Overlay Mode:* In landscape, the keyboard overlays the app with configurable transparency (0–100%) instead of squishing it into a tiny strip. Touches above the keyboard pass through to the app.
- *Split Layout:* Optional split keyboard in landscape with adjustable key width for comfortable thumb typing on wide screens. Gesture typing is automatically disabled in split mode.
